...
def get_single_image_loader(image_path, batch_size=1):
    try:
        transform = transforms.Compose([transforms.Resize((224, 224)), transforms.ToTensor(), transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])])
        image = Image.open(image_path).convert('RGB')
        image_tensor = transform(image).unsqueeze(0)
        single_image_loader = torch.utils.data.DataLoader(image_tensor, batch_size=batch_size)
        return single_image_loader
    except Exception as e:
